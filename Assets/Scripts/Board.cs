...
public class Board : MonoBehaviour
{
        ...
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                if (!blankSpaces[i, j])
                {
                    ...
                    GameObject backgTile = Instantiate(tilePrefab, tempPosition, Quaternion.identity) as GameObject;
                    backgTile.transform.parent = this.transform;
                    backgTile.name = "(" + i + "i" + j + ")";
    ...
    private bool ColumnOrRow()
    {
        ...
        if (firstPiece != null)
        {
            ...
            {
                Dots dots = currentPiece.GetComponent<Dots>();
                if (dots.row == firstPiece.row)
                {
                    numberHor++;
                }
                if (dots.column == firstPiece.column)
                {
                    numberVer++;
                }
            }
